...
#CALCOLO TEMPO PLI
    ...
    mu = []
    b,c = [12,35,47,21,16,46,20,4,11,23,4,11,23], [[8,7],[6,1],[1,4]]
    a = len(b)
    t = time.time()
    m, total_pli = solver.solve_model(solver.pli_implementation(a,b,c, 0)[0])
    total_pli = time.time() - t
    t = time.time()
    xz = branch_and_bound.bb_implementation(a,b,c, [0,0,0])
    total = time.time() - t
                # for i in range (5,16):
